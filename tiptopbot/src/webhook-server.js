 ...
 *   GET  /webhook   — verification handshake (hub.* query params)
 *   POST /webhook   — message callbacks
 *
 * If the production deployment instead routes webhooks through Laravel first,
 * Laravel forwards the JSON body to:
 *   POST /inbound   — same body as /webhook, signature already verified
 *
 * Both routes flow into the same handleEvent() pipeline.
 */
...
function verifySignature(req) {
    if (!APP_SECRET) {
        return true; // dev: signature check disabled
    }
