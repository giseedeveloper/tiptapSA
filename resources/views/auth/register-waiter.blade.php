<x-guest-layout title="TIPTAP | Waiter Registration" :hero-background="true" :wide="true">
    @php
        $initialStep = 1;
    ...
    <div class="max-w-xl mx-auto">
        <div class="text-center mb-6">
            ...
            <h1 class="text-2xl sm:text-3xl font-black text-[#12141C] tracking-tight">Register as Waiter</h1>
            <p class="text-[#64708B] text-sm mt-2 max-w-sm mx-auto leading-relaxed">
                4 quick steps — get your unique number. A manager will link you to a restaurant.
            </p>
        </div>
        ...
        <p id="step-label" class="text-center text-xs font-bold uppercase tracking-wider text-[#64708B] mb-6">Step 1 of 4</p>
        ...
        <form id="waiter-registration-form" method="POST" action="{{ route('waiter.register.store') }}">
            ...
            <div class="step step-active" data-step="1">
                <div class="{{ $bubbleClass }}">Let's start with your full name.</div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="first_name" class="text-[10px] font-bold uppercase tracking-wider text-[#64708B] mb-1.5 block">First name</label>
                        <input id="first_name" type="text" name="first_name" value="{{ old('first_name') }}" required autofocus class="{{ $inputClass }}" placeholder="First name">
                        <x-input-error :messages="$errors->get('first_name')" class="mt-1.5 text-xs" />
                    </div>
                    <div>
                        <label for="last_name" class="text-[10px] font-bold uppercase tracking-wider text-[#64708B] mb-1.5 block">Last name</label>
                        <input id="last_name" type="text" name="last_name" value="{{ old('last_name') }}" required class="{{ $inputClass }}" placeholder="Last name">
            ...
            <div class="step step-hidden" data-step="2">
                <div class="{{ $bubbleClass }}">We need your email and phone number.</div>
                <div class="space-y-4">
                    <div>
                        <label for="email" class="text-[10px] font-bold uppercase tracking-wider text-[#64708B] mb-1.5 block">Email</label>
                        <input id="email" type="email" name="email" value="{{ old('email') }}" required class="{{ $inputClass }}" placeholder="you@example.com">
                        <x-input-error :messages="$errors->get('email')" class="mt-1.5 text-xs" />
                    </div>
                    <div>
                        <label for="phone" class="text-[10px] font-bold uppercase tracking-wider text-[#64708B] mb-1.5 block">Phone number</label>
                        <input id="phone" type="text" name="phone" value="{{ old('phone') }}" required class="{{ $inputClass }}" placeholder="{{ config('tiptap.phone_local_example', '071 234 5678') }}">
            ...
            <div class="step step-hidden" data-step="3">
                <div class="{{ $bubbleClass }}">Where are you based? (Optional — you can skip this step.)</div>
                <div>
                    <label for="location" class="text-[10px] font-bold uppercase tracking-wider text-[#64708B] mb-1.5 block">Location</label>
                    <input id="location" type="text" name="location" value="{{ old('location') }}" class="{{ $inputClass }}" placeholder="e.g. Cape Town">
                    <x-input-error :messages="$errors->get('location')" class="mt-1.5 text-xs" />
                </div>
            </div>

            <div class="step step-hidden" data-step="4">
                <div class="{{ $bubbleClass }}">Finally — set a strong password (at least 8 characters).</div>
                <div class="space-y-4">
                    <div>
                        <label for="password" class="text-[10px] font-bold uppercase tracking-wider text-[#64708B] mb-1.5 block">Password</label>
                        <input id="password" type="password" name="password" required class="{{ $inputClass }}" placeholder="Password">
                        <x-input-error :messages="$errors->get('password')" class="mt-1.5 text-xs" />
                    </div>
                    <div>
                        <label for="password_confirmation" class="text-[10px] font-bold uppercase tracking-wider text-[#64708B] mb-1.5 block">Confirm password</label>
                        <input id="password_confirmation" type="password" name="password_confirmation" required class="{{ $inputClass }}" placeholder="Confirm password">
                    </div>
                </div>
            </div>
            ...
            <p class="text-center text-[#64708B] text-xs mt-3">Free · You receive your unique code instantly</p>
        </form>
