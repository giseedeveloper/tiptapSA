...
class WhatsAppSetupDoctor extends Command
{
    ...
    protected $description = 'Check WhatsApp Cloud API + tiptopbot configuration for TAPTAP_sauth';

    public function handle(): int
    {
        $this->info('TIPTAP_sauth — WhatsApp Cloud API setup check');
        $this->newLine();
        ...
        $botWebhook = WhatsAppBotUrls::botWebhookUrl();
        if ($botWebhook !== null) {
            $this->newLine();
            $this->line('Alternative — Meta → bot directly:');
            $this->line('  '.$botWebhook);
        }

        $this->newLine();
        $this->line('tiptopbot/.env must mirror the same Meta vars + NOTIFY_SECRET + BOT_TOKEN.');
        $this->line('Run bot: cd tiptopbot && npm install && npm start  (or docker compose up -d)');
